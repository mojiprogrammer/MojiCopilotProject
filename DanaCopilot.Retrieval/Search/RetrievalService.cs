...
namespace DanaCopilot.Retrieval.Search
{
    public class RetrievalService: IRetrievalService
    {
        ...
        public async Task<RetrievalContext> GetContextAsync(string question, CancellationToken cancellationToken = default)
        {
            var results = await _sqlSearch.SearchAsync(question);

            var context = _builder.Build(results);

            var confidence = _scorer.Calculate(results);
